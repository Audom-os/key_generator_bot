...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "👋 សួស្តី! សូមផ្ញើ **Machine ID** របស់កុំព្យូទ័រដែលអ្នកចង់ Activate (វាជាលេខកូដ 24 តួ):"
    )
...
async def get_machine_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # ការត្រួតពិនិត្យភាពត្រឹមត្រូវ (Verification) ងាយៗ
    # នៅក្នុងកូដ C# key គឺ 24 តួ និងប្រើតួអក្សរជាក់លាក់
    if len(machine_id) != 24 or not all(c.isalnum() or c in 'KMX' for c in machine_id):
        await update.message.reply_text(
            "Machine ID ហាក់ដូចជាមិនត្រឹមត្រូវ។ វាគួរតែមាន 24 តួអក្សរ។\n"
            "សូមព្យាយាមផ្ញើ Machine ID ម្តងទៀត៖"
        )
